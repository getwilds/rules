...
class WildsRegistry(object):
    ...
    def load_data(self):
        with urllib.request.urlopen(REGISTRY_URL) as url:
            ...
            self.updated = dateutil.parser.parse(data["updated"]).strftime(
                "%Y-%m-%d"
            )
    ...
    def partition(self):
        ...
        self.experimental = [
            w for w in self.all if w["badge_status"] == "experimental"
        ]
        self.prototype = [
            w for w in self.all if w["badge_status"] == "prototype"
        ]
        self.stable = [w for w in self.all if w["badge_status"] == "stable"]
        self.deprecated = [
            w for w in self.all if w["badge_status"] == "deprecated"
        ]
